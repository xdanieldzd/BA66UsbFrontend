...
namespace BA66UsbFrontend.Content.Weather
{
	public class WeatherSubContent : OpenWeatherMapContent
	{
		...
		public override async void Update(Encoding textEncoding, Size displaySize)
		{
			...
			var dataString2 = "\x1B[2;1H" + $"Min{$"{weatherReport?.TempMin:0.0}{unitSuffix}",7}   ¤ {weatherReport?.SunriseTime.TimeOfDay:hh':'mm}".PadRight(displaySize.Width)[..displaySize.Width];
			var dataString3 = "\x1B[3;1H" + $"Max{$"{weatherReport?.TempMax:0.0}{unitSuffix}",7}   ⌂ {weatherReport?.SunsetTime.TimeOfDay:hh':'mm}".PadRight(displaySize.Width)[..displaySize.Width];
